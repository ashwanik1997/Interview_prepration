// Includes #include "stm32f4xx_hal.h" #include <stdbool.h>
...
htim3.Init.Period = 255;           // 8-bit PWM
...
htim9.Instance = TIM9;
htim9.Init.Prescaler = 83;      // 1 MHz
htim9.Init.CounterMode = TIM_COUNTERMODE_UP;
htim9.Init.Period = 24;         // 40 kHz
...
switch (event)
{
    ...
    case TimerTickEvent:
        if (rxMatchingTimer < UINT16_MAX)
            rxMatchingTimer++;
        if (rxMatchingTimer > 5000) // 5 seconds
            audiowrite(true);
        break;

    case ValidStatusByteReceived:
        rxMatchingTimer = 0;
        if ((eventData & 0x01) != 0)
            audiowrite(true);
        else
...
// Main application int main(void) { HAL_Init(); SystemClock_Config(); MX_TIM3_Init(); MX_TIM9_Init();
